...
local lqdFuel is 0.
LOCK lqdFuel to FuelTank(Booster_tag,"check"). //Fuel left in the BOOSTER stage => tanks have to have specific tag name
...
set hoverPID:setPOINT to 3. // to slow down ABOVE landing pad
...
//		print "Booster  dV: " + round(deltaVbooster(), 1) + "          " at (3,27).			
	...
	else if runmode = 3
	{
		if ADDONS:TR:HASIMPACT = true { //If ship will hit ground
			set steeringDir to targetDir - 180. //point towards landing site
			set steeringPitch to 0.
			if vang(heading(steeringDir,steeringPitch):VECtoR, SHIP:FACING:VECtoR) < 45 {  //wait until pointing in right direction
	...
	else if runmode = 3.5 //let's stabilze Booster before re-entry
	{
		...
		wait 10. //to stabilze booster before swith off RCS
		RCS off.
	...
	else if runMode = 4  //Glide rocket to landing site.
	{
		...
		when radar < safe_altitude AND SHIP:VERTICALSPEED < -20 THEN {//When there is barely enough time to slow down.
			RCS on.
			set runmode to 5.
		}
	}
	else if runmode = 5 // Suicide burn (slowing down to safe speed)
	{
		...
		set AltVel_PID:setpoint to radar+25. // landing burn
		If radar > Vdist { //in case we over water use radar altitude instead of laser
			set VelThr_PID:setpoint to AltVel_PID:update(missionT, radar+25). //Vdist). //lower ship down while flying to landing site
		} else {
	...
	if runmode >= 2.2 {
		...
		print "Fuel left:            " + round(lqdFuel, 2) + "     " at (3, 29).
		print "dV left:              " + round(deltav(), 1) + " m/s                     	  " at (3, 30).
