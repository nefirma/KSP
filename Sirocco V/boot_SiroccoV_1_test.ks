...
//configuration
local landReq is 1. // landing position. 1 - RTLS, 0 if landing is not required => check this settings below
...
if landingSite = 0 { 
	...
	lock MECO to 100 + (groundspeed * .7). // amount of fuel needed for boostback and landing. 
}
...
// preparing PID loops
// --- landing loops ---
local laser_height to 1.7. //height of Laser module above ground level with deployed legs
...
local AltVel_PID is pidloop(0.2, 0, 0.04, -250, 250). // was 280 /calculating desired velocity during landing
...
until runmode = 0 {
	...
	if runmode = 1 // keep checking if stage 1 separated
	{
		...
		if lqdFuel:enabled = false { // check if liquid fuel has been cut, indicating staging
	...
	else if runMode = 4  //Glide rocket to landing site.
	{
		...
		when radar < 300 AND SHIP:VERTICALSPEED < -5 THEN {//When there is barely enough time to slow down.
			RCS on.
			set runmode to 5.
		}
	}
	else if runmode = 5 // Suicide burn (slowing down to safe speed)
	{
		...
		set AltVel_PID:setpoint to radar+5. // landing burn
	...
	else if runmode = 7 // In case we misss a bit - powered flight to landing site 
	{
		...
		when geoDistance(SHIP:GEOPOSITION, landingSite) < 5 then { //When it is over the launch pad
			set runmode to 8.
		}
	}
	else if runmode = 8 // final touchdown at safe speed
	{
		...
		set cardVelCached to cardVel().
		steeringPIDs().
		set climbPID:setPOINT to MAX((Vdist-0.1), 1.4) * -1.
		PRINT "climbPID:setPOINT: " + climbPID:setPOINT at(3,39).
		set tval to climbPID:UPDATE(TIME:SECONDS, SHIP:VERTICALSPEED).
		if (ship:status = "Landed") or (Vdist<1.5) { // checking if we have landed already
			set runmode to 0.
			LOCK throttle to 0.
...
//		print "    " + round(landingSite, 5) + "          " at (3,23).
		...
		print "Target Dist:          " + round(targetDist, 3) + "m     " at (3, 28).
		print "Steer Pitch:          " + round(steerPitch, 2) + "     " at (3, 29).
		print "Steer Yaw:            " + round(steerYaw, 2) + "     " at (3, 30).
		print "Impact Time:          " + round(impactTime, 2) + "s     " at (3, 31).
		print "Altitude:             " + round(radar,2) + "m			" at (3, 32).
		print "Laser Altitude:       " + round(Vdist,2) + "m			" at (3, 33).
	}
