...
parameter _Inclination is 30.//default orbit inclination 0
parameter _LAN is 1. //Default LAN
...
local startT is 0.
local launchT is 0.
local UpperTank is ship:partstagged("UpperTank")[0]. // NAME of upperstage fuel tank. 
local FuelLeft is UpperTank:resources[0]:amount.
local head is 0.
...
//lock dMECO to 100 + (groundspeed * .7). // amount of fuel needed for boostback and landing.

//when (BoosterFuelLeft+70) <= dMECO then {  // while staging let's steer to prograde
	...
	else if runmode = 3 // Initiating pitch-over
	{
		if curAlt >= 20000 { //we passed dense air
			...
			if airspeed >= 70 { // from that speed rocket will continue pitching relative to altitude
				set pitch to max(0, 88 * (1 - ship:altitude / 47000)).  // here you may ajust ascending profile

//				set pitch to max(90-(((ship:altitude-curAlt)/(turnEnd-curAlt))^turnExp)*90, 0).
...
//			set pitch to max(0,90*((1-ship:altitude/turnEnd)^turnExp)).
			set pitch to max(0, 88 * (1 - ship:altitude / 47000)). 
